...
extension AboutBoxViewController {
    static func freshController() -> AboutBoxViewController {
        let storyboard = NSStoryboard(name: "AboutBox", bundle: nil)
        let identifier = "AboutBox Controller"

        guard let viewcontroller = storyboard.instantiateController(withIdentifier: identifier) as? AboutBoxViewController else {
            fatalError("Why cant i find AboutBox Controller? - Check AboutBox.storyboard")
        }
